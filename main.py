...
def connect_as_creator(username_creator, password_creator, main_db, cr_dr_func):
    engine = create_engine("postgresql://{}:{}@localhost:5432/{}".format(username_creator, password_creator, main_db),
                           echo=True)
...
def create_database(db_name, username, cr_dr_func):  # название базы данных и имя пользователя, который будет ей пользоваться
...
def drop_database(db_name, cr_dr_func):
    cursor = connect_as_creator('db_creator', 'db_creator', 'lab_db', cr_dr_func)
    cursor.execute('SELECT drop_db(\'{}\')'.format(db_name))  # запускаем удаление


def connect_as_user(username, password, db_name, functions):
...
def add_to_flower(connection, in_id, in_name, in_provider, in_date, in_color, in_worker,
							  in_amount, in_value):
    cursor = connection.cursor()
    cursor.execute('SELECT add_to_flower({}, \'{}\', {}, \'{}\', \'{}\', {}, {}, {})'.format(in_id, in_name, in_provider, in_date, in_color, in_worker,
							  in_amount, in_value))
    connection.commit()
...
def update_flower(connection, in_id, in_name, in_provider, in_date, in_color, in_worker,
							  in_amount, in_value):
    cursor = connection.cursor()
    cursor.execute('SELECT update_flower({}, \'{}\', {}, \'{}\', \'{}\', {}, {}, {})'.format(in_id, in_name, in_provider, in_date,
                                                                                  in_color, in_worker,
                                                                                  in_amount, in_value))
    connection.commit()
...
add_to_provider(conn_u, 2,	'БАЗА ЦВЕТОВ 24', 'Ленинский', 0)  # объект connection используется для всех функций работы с бд
add_to_worker(conn_u, 2,	'Илья Дорошенко',		'Автозаводский',	31000)
